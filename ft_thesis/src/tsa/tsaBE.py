...
class tsaBE(object):
    ...
    LISTENING_IP = '127.0.0.1'  #'10.0.0.101'
    LISTENING_PORT = 9093
    ...
    def __init__ (self):
        ...
        #   - the policychain without the sender host (i.e. for the example above, this cell has this value: [u'm8', u'm3']).
        #   - the vlan id
        #   - the full flow of switches for this policychain. An example to full flow for the example above can be:
        #     [u'$h1', u'000000000000000b', u'000000000000000d', u'0000000000000006', u'000000000000000e', u'$m8',
        #      u'000000000000000e', u'0000000000000006', u'000000000000000c']]
        ...
        # {u'h1$m8,m3$tp_dst:3334': [[u'm8', u'm3'], 2,
        # [u'$h1', u'000000000000000b', u'000000000000000d', u'0000000000000006', u'000000000000000e', u'$m8',
        #  u'000000000000000e', u'0000000000000006', u'000000000000000c']]
        self.policyChainKeyToData = {}


        # Each policychain has a pcid (policy chain id). the pcid is used for common language between tsa BE and FE.
        self.nextPcid = 0
        ...
        self.fillMastersToSlavesMapping()
    ...
    def fillMastersToSlavesMapping(self):
        """
        This method read the configuration file named 'mastersSlaves.txt' which is located on the same directory as this
        file, and fill the mapping between the middleboxes to its backup (i.e. in case that the master stop working,
        its backup (slave) becomes the master, and the slave's backup become the slave.
        The assumed format of the configuration file is:
            - Each middlebox in the network should has a different line in the following format:
              <master_host_id> <slave_host_id>
              For example: for host h6, the line should look like:
              6 7
            - In addition, you are allowed to add comments. A comment should be in new line and start with '#' sign.
        """

        with open("mastersSlaves.txt") as f:
            for line in f:
                ...
                splitted = line.split()
                masterMbName = 'm'+splitted[0]
                slaveMbName = 'm'+splitted[1]

                if masterMbName in self.middleboxToSwitchMapping and slaveMbName in self.middleboxToSwitchMapping:
                    self.masterToSlave[masterMbName] = slaveMbName
                else:
                    printLogMsg(logging.DEBUG, "fillMastersToSlavesMapping", "masterMbName (%s) exist? %s" %(str(masterMbName), str(masterMbName in self.middleboxToSwitchMapping)))
                    printLogMsg(logging.DEBUG, "fillMastersToSlavesMapping", "slaveMbName (%s) exist? %s" %(str(slaveMbName), str(slaveMbName in self.middleboxToSwitchMapping)))

        printLogMsg(logging.DEBUG, "fillMastersToSlavesMapping", "The mapping between master middleboxes to their slaves is: %s" %(str(self.masterToSlave)))
    ...
    def runServer(self):
        ...
        while True:
            ...
            if len(buff) > 0:
                ...
                elif command == tsaBE.STOP_MASTER_MIDDLEBOX_CLI:
                    printLogMsg(logging.INFO, "runServer", "processing 'stop master middlebox' command")
                    response = self.handleStopMasterMBCommand(requestArgs)

                elif command == tsaBE.UNREGISTER_MIDDLEBOX_CLI:
                ...
                elif command == tsaBE.REPLACE_MASTER_MB_CLI:
                    printLogMsg(logging.INFO, "runServer", "processing 'replace master mb' command")
                    response = self.handleReplacingMasterMBCommand(requestArgs)
    ...
    def handleAddPolicyCommand(self, requestArgs):
        try:
            ...
            self.installPolicyChainRules(policyChainKey, preparedPolicyChain, vlanId, matchFields, hostSender)
    ...
    def handleRemovePolicychain(self, requestArgs):
        try:
            ...
            return self.removePolicyChain(policyChainKey)
        except:
    ...
    def handleStopMasterMBCommand(self, requestArgs):
        stoppedMBName = requestArgs
        printLogMsg(logging.DEBUG, "handleStopMasterMBCommand", "stoppedMBName is: %s" %(str(stoppedMBName)))

        if stoppedMBName not in self.masterToSlave:
            printLogMsg(logging.DEBUG, "handleStopMasterMBCommand", "stoppedMBName (%s) is not is master to slave mapping!" %(str(stoppedMBName)))
            return self.toJSONResponseFormat(tsaBE.FAILED_STRING, 'Failed to stop master mb.')

        slaveMb = self.masterToSlave[stoppedMBName]
        return self.doHandleReplacingMasterMB(stoppedMBName, slaveMb)



    def handleReplacingMasterMBCommand(self, requestArgs):
        splitted = requestArgs.split()
        printLogMsg(logging.DEBUG, "handleReplacingMasterMBCommand", "splitted len is: %d" %(len(splitted)))

        if len(splitted) == 2:
            oldMbName = splitted[0]
            newMbName = splitted[1]
            return self.doHandleReplacingMasterMB(oldMbName, newMbName)



    def doHandleReplacingMasterMB(self, oldMbName, newMbName):
        printLogMsg(logging.DEBUG, "handleReplacingMasterMBCommand", "mbName is: %s, newMbName is: %s" %(str(oldMbName), str(newMbName)))
        installedSuccessfully = True

        for policyChainKey in self.policyChainKeyToData:
            [policyChain, vlanId, senderHost] = self.policyChainKeyToData[policyChainKey]
            policyChain = policyChain.split(",")

            if oldMbName in policyChain:
                printLogMsg(logging.DEBUG, "handleReplacingMasterMBCommand", "policyChainKey  is %s, and its policychain is: %s" %(str(policyChainKey), str(policyChain)))
                installedSuccessfully &= self.replaceMasterMB(oldMbName, newMbName, policyChainKey)

                if not installedSuccessfully:
                    printLogMsg(logging.DEBUG, "handleReplacingMasterMBCommand", "Failed to replace master mb in policychain: %s" %(str(policyChain)))

        if installedSuccessfully:
            return self.toJSONResponseFormat(tsaBE.SUCCESS_STRING, 'Master mb was replaced successfully.')
        else:
            return self.toJSONResponseFormat(tsaBE.FAILED_STRING, 'Failed to replace master mb.')
    ...
    def installPolicyChainRules(self, policyChainKey, policyChain, vlanId, matchFields, senderHost):
        """
        Note: policyChain is a list of middleboxes names without the sender host
        """
        ...
        printLogMsg(logging.DEBUG, "installPolicyChainRules", "policychain len: %d, policychain: %s" %(len(policyChain), str(policyChain)))
        policyChain = policyChain.split(",")
        printLogMsg(logging.DEBUG, "installPolicyChainRules", "policychain len: %d, policychain: %s" %(len(policyChain), str(policyChain)))

        # install rules from sender packets host (for example, h1) to first mb
        self.installRulesToFirstMB(policyChainKey, policyChain[0], vlanId, senderHost, matchFields)
    ...
    def installRulesToFirstMB(self, policyChainKey, mbName, vlanId, senderHost, matchFields):
        printLogMsg(logging.DEBUG, "installRulesToFirstMB", "Received arguments are: policyChainKey: %s, mbName: %s, vlanId: %s, senderHost: %s, matchFields: %s" %(str(policyChainKey), str(mbName), str(vlanId), str(senderHost), str(matchFields)))

        if senderHost in self.hostsThatSendPackets:
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "senderHost (%s) is in. Data: %s" %(str(senderHost), self.hostsThatSendPackets[senderHost]))
        else:
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "senderHost (%s) is OUT!" %str(senderHost))
        ...
        if mbName in self.middleboxToSwitchMapping:
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "mbName (%s) is in. Data: %s" %(str(mbName), self.middleboxToSwitchMapping[mbName]))
        else:
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "mbName (%s) is OUT!" %str(mbName))

        dstSwitchData = self.middleboxToSwitchMapping[mbName]
        switchDpidToDstMiddlebox = dstSwitchData[0]
        switchPortToDstMiddlebox = dstSwitchData[1]

        printLogMsg(logging.DEBUG, "installRulesToFirstMB", "switchDpidToPacketsSender: %s, switchDpidToDstMiddlebox: %s" %(str(switchDpidToPacketsSender), str(switchDpidToDstMiddlebox)))
        self.printGraph()
        switchesPath = self.findPathBetweenNodes(switchDpidToPacketsSender, switchDpidToDstMiddlebox)
        printLogMsg(logging.DEBUG, "installRulesToFirstMB", "switches path is: %s" %str(switchesPath))
        ...
        for j in range(len(switchesPath) -1):
            outPort = self.getPortToNeighbor(self.graph.getNode(switchesPath[j])['neighbors'], switchesPath[j+1])
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "The leading port of %s to his neighbor %s, is: %s" %(str(switchesPath[j]), str(switchesPath[j+1]), str(outPort)))
            if j == 0:
            ...
            # keep the incoming port for the next switch
            inPort = self.getPortToNeighbor(self.graph.getNode(switchesPath[j+1])['neighbors'], switchesPath[j])
            printLogMsg(logging.DEBUG, "installRulesToFirstMB", "The leading port of %s to his neighbor %s, is: %s" %(str(switchesPath[j+1]), str(switchesPath[j]), str(inPort)))
    ...
    def uninstallPolicyChainRules(self, policyChainKey, senderHost, matchFields):
        ...
        printLogMsg(logging.DEBUG, "uninstallPolicyChainRules", "Done uninstalling rules")
    ...
    def replaceMasterMB(self, oldMbName, newMasterMBName, policyChainKey):
        """
        policyChain is a list of middleboxes names
        vlanId is an integer number
        """

        printLogMsg(logging.INFO, "replaceMasterMB", "Received arguemtns are: oldMbName: %s, newMasterMBName: %s, policyChainKey: %s" %(str(oldMbName), str(newMasterMBName), str(policyChainKey)))
        ...
        try:
            ...
            printLogMsg(logging.DEBUG, "replaceMasterMB", "fullPolicychain is: %s, len is: %d" %(str(fullPolicychain), len(fullPolicychain)))
            # update the policy chain
            fullPolicychain = string.replace(fullPolicychain, oldMbName, newMasterMBName)
            printLogMsg(logging.DEBUG, "replaceMasterMB", "~~ and now fullPolicychain is: %s, len is: %d" %(str(fullPolicychain), len(fullPolicychain)))

            self.installPolicyChainRules(policyChainKey, fullPolicychain, vlanId, matchFields, senderHost)
            printLogMsg(logging.DEBUG, "replaceMasterMB", "new policychain data for policychainKey is: %s" %str(self.policyChainKeyToData[policyChainKey]))

            return True

        except:
            printLogMsg(logging.DEBUG, "replaceMasterMB", "Caught an exception")
            return False



    def removePolicyChain(self, policyChainKey):
        printLogMsg(logging.INFO, "removePolicyChain", "Received arguemtns are: policyChainKey: %s" %(str(policyChainKey)))
...
if len(sys.argv) != 1 and len(sys.argv) != 2:
    print "please use as follow: python TSA.py [debug]"
    print "for example: python TSA.py debug"
else:
    debugMode = False
    for arg in sys.argv:
        if arg == "debug":
