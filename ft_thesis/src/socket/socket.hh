...
private:
  ...
  int readPacketLen(int sockfd);
  int readPacket(int sockfd, unsigned char* packet, int packetLen);
  int checkLength (char* num, int length, int minimalExpectedValue);
  int receiveMsgFromClient (int clientSockfd, int totalReceivedBytes, unsigned char* msg, int maximalReceivedBytes);
